        ...
        <section className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {GUIDES.map((guide) => (
            <article key={guide.slug} className="rounded-[28px] border border-[#e5e8eb] bg-white p-6 shadow-[0_16px_36px_rgba(15,23,42,0.06)]">
              ...
              <p className="mt-3 text-sm leading-7 text-[#4e5968]">{guide.excerpt}</p>
              <div className="mt-4 flex items-center justify-between text-xs text-[#8b95a1]">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <Link
                  ...
                  className="rounded-full bg-[#191f28] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#0f172a]"
                >
                  읽어보기
                </Link>
                <Link
                  href={`/?q=${encodeURIComponent(guide.queryHint)}&source=guide`}
                  className="rounded-full bg-[#eef4ff] px-4 py-2 text-sm font-semibold text-[#3182f6] transition hover:bg-[#dbeafe]"
                >
                  이 주제로 검색
                </Link>
              </div>
