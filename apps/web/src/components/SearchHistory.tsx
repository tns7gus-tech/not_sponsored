...
export default function SearchHistory({ onHistoryClick }: Props) {
  ...
  return (
    <section
      ...
      className="mx-auto mt-12 w-full max-w-4xl rounded-[28px] border border-white/8 bg-[rgba(7,14,26,0.72)] p-5 shadow-[0_18px_50px_rgba(4,10,20,0.18)] backdrop-blur-xl sm:p-6"
    >
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 id="recent-searches-title" className="flex items-center gap-2 text-sm font-medium text-white">
        ...
        <button
          ...
          className="rounded-full border border-white/12 px-3 py-1.5 text-xs font-medium text-slate-100 transition hover:border-white/24 hover:bg-white/8 hover:text-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-white/20"
        >
          전체 삭제
        </button>
      </div>
      ...
      <ul className="flex flex-wrap gap-2.5" aria-label="최근 검색 기록">
        {history.map((item) => (
          <li key={item.jobId}>
            ...
            >
              <span className="text-sm font-medium text-white transition group-hover:text-white">{item.query}</span>
